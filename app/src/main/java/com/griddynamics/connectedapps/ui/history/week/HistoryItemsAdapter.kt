...
class HistoryItemsAdapter(private val metrics: List<MetricChartItem>) :
    RecyclerView.Adapter<HistoryItemsAdapter.WeekHistoryItemViewHolder>() {
    ...
    override fun onBindViewHolder(holder: WeekHistoryItemViewHolder, position: Int) {
        ...
        metric.data.forEach {
            seriesData.add(CustomDataEntry(it.keys.first(), it.values.first().toFloat()))
        }
